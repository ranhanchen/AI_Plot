...
const DEFAULT_ITEM_KEYS = new Set(['AI 剧情推动提示词', 'AI 总结提示词', 'AI 角色生成提示词'])
...
const remarkBeforeEdit = ref('')
const dirtyIds = ref(new Set<number>())

function checkRemarkConflict(remark: string, excludeId?: number): boolean {
  const trimmed = remark.trim()
  if (!trimmed) return false
  return items.value.some(i => i.id !== excludeId && i.remark.trim() === trimmed)
}

function validateRemarkOnBlur(item: SystemConfigItem) {
  const trimmed = item.remark.trim()
  if (!trimmed) return
  if (checkRemarkConflict(trimmed, item.id)) {
    item.remark = remarkBeforeEdit.value
    appStore.showToast(`备注「${trimmed}」已存在，不可重复`, 'error')
  }
  remarkBeforeEdit.value = ''
}
...
async function loadItems() {
  ...
  for (const d of DEFAULT_ITEMS) {
    if (!fromDb.some(i => i.key === d.key)) {
      ...
      fromDb.push({ id, key: d.key, value: d.value, remark: d.key, sortOrder: Date.now(), createdAt: Date.now() } as SystemConfigItem)
    }
  }
...
async function confirmRestore() {
  ...
  const def = DEFAULT_ITEMS.find(d => d.key === restoreTarget.value!.key)
  if (def) {
    restoreTarget.value.value = def.value
...
function toggleExpand(id: number) {
  if (expandedId.value !== id) {
    ...
    if (item) remarkBeforeEdit.value = item.remark
  }
      ...
      <div v-if="expandedId === item.id" class="px-4 pb-4 space-y-3 border-t border-[var(--color-border)] pt-3 bg-[var(--color-surface-hover)]/30">
        <div>
          ...
          <input
            ...
            @blur="validateRemarkOnBlur(item)"
          />
        </div>
        <div>
